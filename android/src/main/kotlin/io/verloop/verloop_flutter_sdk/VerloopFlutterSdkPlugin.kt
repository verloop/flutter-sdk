    ...
    when (call.method) {
      "setConfig" -> {
        ...
        if (customFields != null) {
          val roomFields = ArrayList<VerloopConfig.CustomField>()
          for ((key, value) in customFields) {
            val customField = VerloopConfig.CustomField()
            customField.key = key
            customField.value = value
            customField.scope = VerloopConfig.Scope.ROOM
            roomFields.add(customField)
          }
          configBuilder =
            configBuilder!!.fields(roomFields)               // These are predefined variables added on room level
        }
        ...
        if (userCustomFields != null) {
          ...
          configBuilder =
            configBuilder!!.fields(fields)               // These are predefined variables added on user level
        }
